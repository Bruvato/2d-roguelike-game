...
func _process(delta):
	if Input.is_action_pressed("fire"):
		if (!_fired and !_reloading):
			...
			var new_bullet: Bullet = spawner_component.spawn(shooting_point.global_position, shooting_point.global_rotation)
			new_bullet.move_component.velocity = Vector2.RIGHT.rotated(global_rotation)
			
			get_parent().velocity = global_position.direction_to(%"ShootingPoint".global_position) * recoil_force
